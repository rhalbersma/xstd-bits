...
#include <xstd/bits/from_bits.hpp>      // from_bits, from_bits_t
...
BOOST_AUTO_TEST_SUITE(FromBits)

namespace {

template<class T>
concept deduces_from_bits_of = requires (T const& value) { xstd::basic_bit_array(xstd::from_bits, value); };
...
{
        static_assert(std::is_default_constructible_v<xstd::from_bits_t>);
        static_assert(not braces_convert<xstd::from_bits_t>);
        static_assert(std::same_as<decltype(xstd::from_bits), xstd::from_bits_t const>);
        BOOST_CHECK(true);
}
...
BOOST_AUTO_TEST_CASE(AnIntegerDeducesItsOwnWidth)
{
        ...
        constexpr auto a = xstd::basic_bit_array(xstd::from_bits, word);
        static_assert(std::same_as<decltype(a), xstd::basic_bit_array<std::uint16_t, 16> const>);
        static_assert(a == xstd::basic_bit_array<std::uint16_t, 16>::from_bits(word));
        static_assert(a[0] and not a[1] and a[2] and a[15]);

        constexpr auto s = xstd::basic_bit_static_set(xstd::from_bits, word);
        static_assert(std::same_as<decltype(s), xstd::basic_bit_static_set<std::uint16_t, 16> const>);
        static_assert(s == xstd::basic_bit_static_set<std::uint16_t, 16>::from_bits(word));
        static_assert(s.size() == 3UZ and s.contains(15UZ));

        constexpr auto b = xstd::basic_bitset(xstd::from_bits, word);
...
BOOST_AUTO_TEST_CASE(AnArrayOfBlocksDeducesTheirWidth)
{
        ...
        constexpr auto a = xstd::basic_bit_array(xstd::from_bits, blocks);
        static_assert(std::same_as<decltype(a), xstd::basic_bit_array<std::uint8_t, 24> const>);
        static_assert(a[0] and a[23] and a.count() == 2UZ);

        constexpr auto s = xstd::basic_bit_static_set(xstd::from_bits, blocks);
        ...
        constexpr auto b = xstd::basic_bitset(xstd::from_bits, blocks);
        static_assert(std::same_as<decltype(b), xstd::basic_bitset<std::uint8_t, 24> const>);
        static_assert(b.test(23UZ));
        BOOST_CHECK((a == xstd::basic_bit_array<std::uint8_t, 24>::from_bits(blocks)));
}
...
{
        static_assert(deduces_from_bits_of<std::uint64_t>);
        static_assert(deduces_from_bits_of<std::array<std::uint32_t, 2>>);
        static_assert(not deduces_from_bits_of<int>);
        static_assert(not deduces_from_bits_of<std::array<int, 2>>);
        static_assert(bitset_deduces_from<std::uint32_t>);
        static_assert(not bitset_deduces_from<int>);
        static_assert(not std::is_constructible_v<xstd::bit_vector, xstd::from_bits_t, std::uint64_t>);
        ...
        constexpr auto wide = xstd::basic_bitset(xstd::from_bits, xstd::uint128{1} << 100U);
