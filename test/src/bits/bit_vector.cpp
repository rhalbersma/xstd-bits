...
{
        static_assert(std::same_as<T, xstd::sequence_adaptor<xstd::detail::bits::block_vector<std::uint8_t>, xstd::ownership::owns, false>>);
...
BOOST_AUTO_TEST_CASE(ItGrowsLikeAStdVector)
{
        ...
        BOOST_CHECK_EQUAL(v.max_size(), xstd::detail::bits::block_vector<std::uint8_t>().max_size());
...
BOOST_AUTO_TEST_CASE(TheOwnerHashesAndTheViewDoesNot)
{
        ...
        static_assert(not std::is_default_constructible_v<std::hash<xstd::bit_span<xstd::detail::bits::block_vector<std::uint8_t>>>>);
}
