...
{
        constexpr auto word = xstd::bit_sequence_adaptor(xstd::from_bits, std::uint16_t{0b1000'0000'0000'0101});
        ...
        constexpr auto words = xstd::bit_sequence_adaptor(xstd::from_bits, std::array<std::uint8_t, 3>{0x01, 0x00, 0x80});
        ...
        BOOST_CHECK((words == xstd::basic_bit_array<std::uint8_t, 24>::from_bits(std::array<std::uint8_t, 3>{0x01, 0x00, 0x80})));
}
