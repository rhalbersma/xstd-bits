...
#include <xstd/bits/bit_inplace_set.hpp>  // IWYU pragma: keep; basic_bit_inplace_set, named only under __cpp_lib_inplace_vector
...
{
        static_assert(std::same_as<xstd::bits::detail::owned_bits_t<xstd::basic_bit_static_set<std::uint64_t, 64>>, xstd::bits::detail::owned_bits_t<xstd::bit_set_adaptor<std::array<std::uint64_t, 1>>>>);
        static_assert(std::same_as<xstd::bits::detail::owned_bits_t<xstd::basic_bit_static_set<std::uint8_t, 20>>, xstd::bits::detail::owned_bits_t<xstd::bit_set_adaptor<std::array<std::uint8_t, 3>, 20>>>);
...
#ifdef __cpp_lib_inplace_vector
        static_assert(std::same_as<xstd::bits::detail::owned_bits_t<xstd::basic_bit_inplace_set<std::uint16_t, 48>>, xstd::bits::detail::owned_bits_t<xstd::bit_set_adaptor<std::inplace_vector<std::uint16_t, 3>>>>);
#endif

        // Two names over one storage are two types, and no comparison crosses between them.
        static_assert(not compares_with<xstd::basic_bit_static_set<std::uint8_t, 24>, xstd::bit_set_adaptor<std::array<std::uint8_t, 3>>>);
...
BOOST_AUTO_TEST_CASE(TheAdaptorDeducesFromBits)
{
        ...
        BOOST_CHECK(std::ranges::equal(words, xstd::basic_bit_static_set<std::uint8_t, 24>(xstd::from_bit_storage, std::array<std::uint8_t, 3>{0x01, 0x00, 0x80})));
}
