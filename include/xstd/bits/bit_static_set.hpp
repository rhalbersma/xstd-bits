...
class basic_bit_static_set : public bits::detail::set_adaptor<bits::detail::contiguous_bit_container<std::array<Block, bits::detail::num_blocks_v<Block, N>>, N>, bits::detail::storage::owned, basic_bit_static_set<Block, N>>
{
        using base_type = bits::detail::set_adaptor<bits::detail::contiguous_bit_container<std::array<Block, bits::detail::num_blocks_v<Block, N>>, N>, bits::detail::storage::owned, basic_bit_static_set<Block, N>>;
...
public:
        ...
        [[nodiscard]] constexpr basic_bit_static_set() noexcept
                : basic_bit_static_set(key_compare())
        {}

        [[nodiscard]] constexpr explicit basic_bit_static_set(key_compare const& /* comp */) noexcept
                : base_type()
        {}

        template<std::input_iterator InputIterator>
        [[nodiscard]] constexpr basic_bit_static_set(InputIterator first, InputIterator last, key_compare const& /* comp */ = key_compare())
                : base_type(first, last)
        {}

        template<xstd::container_compatible_range<value_type> R>
        [[nodiscard]] constexpr basic_bit_static_set(std::from_range_t, R&& rg, key_compare const& /* comp */ = key_compare())
                : base_type(std::from_range, std::forward<R>(rg))
        {}

        [[nodiscard]] basic_bit_static_set(basic_bit_static_set const& x) = default;
        [[nodiscard]] basic_bit_static_set(basic_bit_static_set&& x) = default;

        [[nodiscard]] constexpr basic_bit_static_set(std::initializer_list<value_type> il, key_compare const& /* comp */ = key_compare())
                : base_type(il)
        {}

        ~basic_bit_static_set() = default;

        auto operator=(basic_bit_static_set const& x) -> basic_bit_static_set& = default;
        auto operator=(basic_bit_static_set&& x) noexcept -> basic_bit_static_set& = default;
        ...
        [[nodiscard]] constexpr basic_bit_static_set(from_bit_storage_t, B const& b) noexcept
...
basic_bit_static_set(from_bit_storage_t, Block) -> basic_bit_static_set<Block, bit_storage_extent_v<Block> * K>;

// No guide from zero blocks: an empty array names no width worth deducing.
template<xstd::unsigned_integer Block, std::size_t K>
        requires (K != 0)
basic_bit_static_set(from_bit_storage_t, std::array<Block, K>) -> basic_bit_static_set<Block, bit_storage_extent_v<std::array<Block, K>>>;
