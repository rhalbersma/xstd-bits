...
class basic_bit_inplace_vector : public bits::detail::sequence_adaptor<bits::detail::contiguous_bit_container<std::inplace_vector<Block, bits::detail::num_blocks_v<Block, N>>, N>, bits::detail::storage::owned, bits::detail::window::all, basic_bit_inplace_vector<Block, N>>
{
        using base_type = bits::detail::sequence_adaptor<bits::detail::contiguous_bit_container<std::inplace_vector<Block, bits::detail::num_blocks_v<Block, N>>, N>, bits::detail::storage::owned, bits::detail::window::all, basic_bit_inplace_vector<Block, N>>;
...
public:
        ...
        [[nodiscard]] basic_bit_inplace_vector() noexcept = default;

        [[nodiscard]] constexpr explicit basic_bit_inplace_vector(size_type n)
                : base_type(n)
        {}

        [[nodiscard]] constexpr basic_bit_inplace_vector(size_type n, bool const& value)
                : base_type(n, value)
        {}

        template<std::input_iterator InputIterator>
        [[nodiscard]] constexpr basic_bit_inplace_vector(InputIterator first, InputIterator last)
                : base_type(first, last)
        {}

        template<xstd::container_compatible_range<bool> R>
        [[nodiscard]] constexpr basic_bit_inplace_vector(std::from_range_t, R&& rg)
                : base_type(std::from_range, std::forward<R>(rg))
        {}

        [[nodiscard]] basic_bit_inplace_vector(basic_bit_inplace_vector const& x) = default;
        [[nodiscard]] basic_bit_inplace_vector(basic_bit_inplace_vector&& x) noexcept = default;

        [[nodiscard]] constexpr basic_bit_inplace_vector(std::initializer_list<bool> il)
                : base_type(il)
        {}

        ~basic_bit_inplace_vector() = default;

        auto operator=(basic_bit_inplace_vector const& x) -> basic_bit_inplace_vector& = default;
        auto operator=(basic_bit_inplace_vector&& x) noexcept -> basic_bit_inplace_vector& = default;

        // Not in [inplace.vector.cons]: flat_set's container constructor under the bit-storage tag.
        [[nodiscard]] constexpr basic_bit_inplace_vector(from_bit_storage_t, block_container_type blocks) noexcept
...
basic_bit_inplace_vector(from_bit_storage_t, std::inplace_vector<Block, K>) -> basic_bit_inplace_vector<Block, bit_storage_extent_v<Block> * K>;

// The adaptor named by its storage stays the door for an inplace_vector of blocks passed to it directly.
template<xstd::unsigned_integer Block, std::size_t K>
bit_sequence_adaptor(from_bit_storage_t, std::inplace_vector<Block, K>) -> bit_sequence_adaptor<std::inplace_vector<Block, K>>;

namespace aligned {

template<xstd::unsigned_integer Block, std::size_t N>
using basic_bit_inplace_vector = xstd::basic_bit_inplace_vector<Block, xstd::align_up(N, static_cast<std::size_t>(std::numeric_limits<Block>::digits))>;
