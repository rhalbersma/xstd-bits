...
// P2278R4's two aliases, transcribed from [const.iterators.alias] and [ranges.syn], constraints included: the paper defines them by a formula, so this is a definition and not an approximation of one. Unconditional, and in a namespace of its own so that a library shipping the paper does not shadow it but is checked against it. [design.md#the-const-reference]
namespace fallback {
